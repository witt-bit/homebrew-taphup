#!/usr/bin/env bash
# Upgrader/imfile.sh
# 用途：自动获取 imfile 最新 release（或使用传入版本），下载 macOS dmg，计算 sha256，
# 并更新仓库中的 Casks/imfile.rb 中的 version 与 sha256（arm/intel）。
#
# 用法：
#   ./Upgrader/imfile.sh               # 获取 upstream 最新 release 并更新本地 Cask 文件（不会自动提交）
#   ./Upgrader/imfile.sh 2.0.5          # 指定版本号（可以带或不带前缀 v）
#   ./Upgrader/imfile.sh --commit       # 执行更新并创建本地分支、提交并打印 push/PR 指令
#   ./Upgrader/imfile.sh --commit --push  # 还会把分支 push 到 origin
#
# 依赖：curl, jq, perl, shasum 或 sha256sum, git (如使用 --commit)
set -euo pipefail

REPO_OWNER="imfile-io"
REPO_NAME="imfile-desktop"
CASK_PATH="Casks/imfile.rb"
...
Usage: $0 [version] [--commit] [--push]
  --commit   create branch and commit changes locally (branch name: update/imfile-VERSION-TIMESTAMP)
...
INTEL_NAME="imFile-${VERSION}.dmg"
ARM_NAME="imFile-${VERSION}-arm64.dmg"
...
if [[ $COMMIT -eq 1 ]]; then
  ...
  BRANCH="update/imfile-${VERSION}-$(date +%s)"
  git checkout -b "${BRANCH}"
  git add "${CASK_PATH}"
  git commit -m "imfile: update to ${VERSION} (update sha256)"
